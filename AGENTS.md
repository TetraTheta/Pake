...
The workflow can also be triggered manually via `workflow_dispatch` with an option to publish Docker independently. App builds are intentionally manual-input GitHub Actions or local builds; do not reintroduce a default-app build matrix.
...
- **CLI Documentation** (`docs/cli-usage.md`): include **all** CLI parameters with detailed usage examples.
