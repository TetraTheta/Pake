...
async function mergeIcons(options, name, tauriConf, platform, safeAppName) {
    ...
    if (options.systemTrayIcon.length > 0) {
        if (platform !== 'darwin') {
            ...
            trayIconPath = await resolveSystemTrayIconPath(options.systemTrayIcon, trayIconPath, safeAppName);
        }
    }
...
async function detectPackageManager() {
    ...
    if (pnpmMajor !== null &&
        ...
        logger.warn(`✼ Detected pnpm ${normalizedPnpmVersion}, but Pake is pinned to ${packageJson.packageManager}; using the installed pnpm anyway.`);
    }
...
`;
    return program$1
        ...
        .hideHelp())
        .addOption(new Option('--tray <mode>', 'Tray visibility: always, minimized (lazy), or never')
        ...
        .addOption(new Option('--enable-find', 'Enable cross-platform in-page Find UI with Cmd/Ctrl+F/G shortcuts')
