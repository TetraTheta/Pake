...
The tray uses the embedded app icon by default. Use `--tray always` to keep it visible, `--tray minimized` to create it on demand only while minimized to tray, or `--tray never` to disable tray behavior. On macOS only, `--system-tray-icon` can set a separate menu bar icon.
